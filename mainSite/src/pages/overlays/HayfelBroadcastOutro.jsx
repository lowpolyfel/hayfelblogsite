...
/* ============================================================
   HAYFEL BROADCAST SYSTEM  ·  v2  ·  CIERRE
   Loop de despedida para Twitch. OBS: Browser Source 1920x1080 @ 60fps.
...
const CONFIG = {
  ...
  channelNo: "03",
  est: "2026",
  standbyTitle: "TRANSMISIÓN FINALIZADA",
  standbySub: "GRACIAS POR VER",
  ...
  index: [
    ["05", "CIERRE", "AHORA"],
    ["04", "CHARLA / CHAT", "HECHO"],
    ["03", "SESIÓN PRINCIPAL", "HECHO"],
    ["02", "APERTURA", "HECHO"],
    ["01", "CUENTA REGRESIVA", "HECHO"],
  ],
};
...
const BOOT_LINES = [
  ...
  { k: "TAPE", v: "E-180 · USED 133MIN" },
  { k: "REWIND", v: "AUTO ····· COMPLETE" },
  { k: "SIGNAL", v: "RELEASED" },
  ...
  return (
    <div className="hb-scene hb-boot">
      <div className="hb-boot-head">
        ...
        {n > 0 && <span className="hb-boot-title">STOP ■ AUTO</span>}
      </div>
      ...
      {n >= BOOT_LINES.length + 2 && <div className="hb-boot-ok">FIN DE CINTA</div>}
    </div>
  );
}
    ...
    <div className="hb-scene hb-lock">
      <div className="hb-lock-inner">
        <div className={`hb-lock-top ${n > 0 ? "on" : ""}`}>
          ...
          <span className="hb-jp">放送終了</span>
          <span className="hb-jp">また次回</span>
        </div>
  ...
  return (
    <div className="hb-scene hb-idx">
      <div className={`hb-idx-head ${n > 0 ? "on" : ""}`}>
        <span>RESUMEN DE CINTA</span>
        <span className="hb-jp">本日の内容</span>
      </div>
...
function SceneStandby() {
  ...
  return (
    <div className="hb-scene hb-standby">
      ...
      <div className={`hb-sb-jp ${n > 2 ? "on" : ""}`}>また次回{".".repeat(dots)}</div>
...
export default function HayfelBroadcastOutro() {
